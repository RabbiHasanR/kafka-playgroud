...
- [ ] **T1** — Add `psycopg[binary]` to `pyproject.toml`. Rebuild the image and confirm all
...
- [ ] **T2** — Write `scripts/state_schema.sql`: `order_fold` with
...
- [ ] **T3** — Write `scripts/apply_state_schema.sh`: apply T2's file with `psql`, taking the
...
- [ ] **T4** — Add the `postgres:17-alpine` service to `docker-compose.yml` with a `pg_isready`
...
- [ ] **T5** — Extend `Settings` in `config.py` with `state_backend` (`memory` default),
...
- [ ] **T6** — Create `consumer/state.py` with the `StateStore` protocol (`load`, `save`,
  `forget`, `close`) and the frozen `SaveOutcome` carrying `applied` and `handled_count`. Import
  `OrderFold` from `runtime.py` and **do not add `handled_count` to it** — the domain fold keeps
  no opinion about persistence, which is what leaves `apply_event()` untouched.
  — *R3.19* — D2
- [ ] **T7** — Implement `MemoryStateStore` by lifting `ServiceConsumer._folds` and
...
- [ ] **T8** — Implement `PostgresStateStore.__init__`: connect with `psycopg` v3,
  ...
  — *R3.21* — D9, D10, D13
- [ ] **T9** — Implement `load()` and `forget()`. `load()` serves from the partition-keyed cache
...
- [ ] **T10** — Implement `save()` as the single guarded upsert from D6: fold columns advance
  only when the incoming sequence is higher, `handled_count` increments unconditionally,
  `RETURNING last_sequence, handled_count`. Derive `applied` by comparing the returned sequence
  with the one sent. One statement — no `SELECT … FOR UPDATE`, no read-modify-write, no
  application lock. — *R3.5, R3.11, R3.13* — D6
- [ ] **T11** — Handle a store failure during consumption: log at `ERROR` with a stable
...
- [ ] **T12** — Inject a `StateStore` into `ServiceConsumer`; delete `_folds` and `_drop_folds`;
...
- [ ] **T13** — Implement the write order in `_handle_message`: state write then offset commit
...
- [ ] **T14** — Implement the crash lever with **`os._exit(1)`**, fired after the state write or
...
- [ ] **T15** — When `SaveOutcome.applied` is false, log `DUPLICATE_ABSORBED` at `WARNING` with
...
- [ ] **T16** — Add the state backend to the startup banner beside the protocol and assignor
  002 put there, so a log excerpt carries which backend produced it. — *R3.23* — D8
- [ ] **T17** — Build the store from settings in `main.py` before the `Consumer` is constructed,
...
- [ ] **T18** — **Restart amnesia is gone.** Place an order, advance it to `PACKED`, restart a
...
- [ ] **T19** — **Rebalance amnesia is gone.** Re-run 002's T17 unchanged: advance several
...
- [ ] **T20** — **A full replay changes nothing.** Snapshot `order_fold` for one group, reset
...
- [ ] **T21** — **The dual-write gap, as a number.** Set `STATE_CRASH_AFTER=state_write` on one
...
- [ ] **T22** — **The other order loses data.** Set `STATE_WRITE_ORDER=offset_first` and
  ...
  problem. — *R3.18*
- [ ] **T23** — **Group isolation survived shared storage.** With all three services running,
...
- [ ] **T24** — **The memory backend still reproduces 002.** Set `STATE_BACKEND=memory` and
...
- [ ] **T25** — **Failing loudly.** With postgres stopped, start a consumer on the `postgres`
...
- [ ] **T26** — **Real violations still fire.** With durable state on, publish a genuinely out
...
- [ ] **T27** — **Host and compose.** Run a consumer from the host against `localhost:9092` and
...
- [ ] **T28** — Write `docs/durable-state.md`: a committed offset is a position and a fold is a
  ...
  — *R3.28, R3.29*
- [ ] **T29** — Add a `README.md` section: running with Postgres, applying the schema (including
