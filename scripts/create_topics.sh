#!/usr/bin/env bash
# Create the topics feature 001 needs (R1.6).
#
# Auto-creation is off in docker-compose.yml (R0.14 / R1.9), so this is a required
# setup step rather than a convenience — producing to a topic that does not exist
# fails loudly instead of silently creating a 3-partition topic behind your back.
#
# Usage: scripts/create_topics.sh [partitions]

set -euo pipefail

TOPIC="${ORDER_EVENTS_TOPIC:-order-events}"
PARTITIONS="${1:-3}"
