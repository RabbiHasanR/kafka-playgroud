...
Everything below runs against the local cluster defined in
[`docker-compose.yml`](../docker-compose.yml). See the
[README](../README.md) for what the project is and how to start it.
...
See [replication.md §6](replication.md).
...
This is the mechanism [spec 006](walkthroughs.md#spec-006--compaction-and-tombstones) builds on, where
`order-snapshot` is a real compacted topic and `DELETE /orders/{id}` writes a real
tombstone. See [compaction-and-tombstones.md](compaction-and-tombstones.md) for
